...
当前版本主要先保证连接、收发、协议解析、消息分发和心跳等基础流程能够正常运行，整体结构保持简单，没有提前加入过多业务模块。

目前的一些实现方式：

* 每个 `IoWorker` 使用独立线程和 `io_context`
* Session 创建后固定归属一个 Worker
* 不为每条 Session 单独创建线程
* Handle 直接运行在 IoWorker 线程
* 同一种消息只创建一个 Handle 实例，因此 Handle 保持无状态
* 暂时没有额外业务线程池
* 心跳只针对当前 Ping / Pong 需求，没有单独实现通用 Timer
* 消息注册目前使用模板和宏完成
...
后续会优先通过实际游戏项目继续验证 NetFrame，再根据实际需求补充功能，例如：
